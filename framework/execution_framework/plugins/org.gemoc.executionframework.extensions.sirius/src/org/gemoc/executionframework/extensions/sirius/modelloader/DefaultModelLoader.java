...
public class DefaultModelLoader implements IModelLoader {
	...
	public Resource loadModelForAnimation(IExecutionContext context) throws RuntimeException {
		...
		if (context.getRunConfiguration().getAnimatorURI() != null) {
			...
			for (Resource r : resourceSet.getResources()) {
				if (r.getURI().equals(context.getRunConfiguration().getExecutedModelURI())) {
					resource = r;
					break;
				}
			}
	...
	protected HashMap<String, String> getnsURIMapping(IExecutionContext context) {
		...
		/*
		 * String xdsmluri = context.getLanguageDefinitionExtension().getXDSMLFilePath(); if
		 * (!xdsmluri.startsWith("platform:/plugin")) xdsmluri = "platform:/plugin" + xdsmluri; Object o =
		 * EMFResource.getFirstContent(xdsmluri); if(o != null && o instanceof LanguageDefinition){ LanguageDefinition
		 * ld = (LanguageDefinition)o; ... }
		 */
		return nsURIMapping;
	}
	...
	class MelangeURIConverter extends ExtensibleURIConverterImpl {
		...
		public InputStream createInputStream(URI uri, Map<?, ?> options) throws IOException {
			...
			if (uri.scheme().equals("melange")) {
				...
				if (uri.fileExtension() != null && uri.fileExtension().equals(_fileExtension)) {
			...
			// originalInputStream = super.createInputStream(uriToUse, options);
			// String originalContent = convertStreamToString(originalInputStream);
			// String modifiedContent = originalContent.replace("http://www.gemoc.org/sample/tfsm",
			// "http://tfsmextended");
			// result = new StringInputStream(modifiedContent);
			// }
			// finally
	...
	class XMLURIHandler extends XtextPlatformResourceURIHandler {
		...
		public URI resolve(URI uri) {
			...
			if (resolvedURI.scheme().equals("melange") && resolvedURI.fileExtension().equals(_fileExtension)
				...
				String part3 = resolvedURI.toString().substring(
						lastIndexOfFileExtension + fileExtensionWithPoint.length());
